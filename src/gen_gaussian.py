...
def gen_gs_geom_opt(coords, functional="B3LYP", basis="6-31(d,p)", dispersion=1, solvent="Methanol", charge=0, multiplicity=1, nproc=28,mem=100):
    ...
    with open("ground_state_geom_opt/geom_opt.gjf","w") as f:
        for line in header:
            f.write(line+"\n")
        if dispersion == 1:
            f.write("#n %s/%s empiricaldispersion=gd3 opt freq SCRF=(Solvent=%s)\n" % (functional, basis, solvent))
        else:
            f.write("#n %s/%s opt freq SCRF=(Solvent=%s)\n" % (functional, basis, solvent))
        f.write("\n")
...
def gen_vert_exc(nstates=6, functional="B3LYP", basis="6-31(d,p)", dispersion=1, solvent="Methanol", charge=0, multiplicity=1, nproc=28,mem=100):
    ...
    with open("vert_exc/vert_exc.gjf","w") as f:
        for line in header:
            f.write(line+"\n")
        if dispersion == 1:
            f.write("#n %s/%s empiricaldispersion=gd3 TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s,CorrectedLR)\n" % (functional, basis, nstates, solvent))
        else:
            f.write("#n %s/%s TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s,CorrectedLR)\n" % (functional, basis, nstates, solvent))
        f.write("\n")
        f.write("Excited state corrected LR calculation\n")
        f.write("\n")
        f.write("%d %d\n" % (charge, multiplicity))

def gen_exc_geom(nstates=6, functional="B3LYP", basis="6-31(d,p)", dispersion=1, solvent="Methanol", charge=0, multiplicity=1, nproc=28,mem=100):
    ...
    with open("exc_geom/exc_geom.gjf","w") as f:
        for line in header:
            f.write(line+"\n")
        if dispersion == 1:
            f.write("#n %s/%s empiricaldispersion=gd3 TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s)  opt=CalcFC Freq NoSymm\n" % (functional, basis, nstates, solvent))
        else:
            f.write("#n %s/%s TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s) opt=CalcFC Freq NoSymm\n" % (functional, basis, nstates, solvent))
        f.write("\n")
        f.write("Excited state geometry optimization calculation\n")
        f.write("\n")
        f.write("%d %d\n" % (charge, multiplicity))

def gen_exc_solv(nstates=6, functional="B3LYP", basis="6-31(d,p)", dispersion=1, solvent="Methanol", charge=0, multiplicity=1, nproc=28,mem=100):
    ...
    with open("exc_solv/exc_solv.gjf","w") as f:
        for line in header:
            f.write(line+"\n")
        if dispersion == 1:
            f.write("#n %s/%s empiricaldispersion=gd3 TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s,CorrectedLR,NonEquilibrium=Save)  NoSymm\n" % (functional, basis, nstates, solvent))
        else:
            f.write("#n %s/%s TD=(Nstates=%d,Root=1) Geom=Check Guess=Read SCRF=(Solvent=%s,CorrectedLR,NonEquilibrium=Save) NoSymm\n" % (functional, basis, nstates, solvent))
        f.write("\n")
        f.write("Excited state solvent field calculation\n")
        f.write("\n")
        f.write("%d %d\n" % (charge, multiplicity))

def gen_gs_sp(functional="B3LYP", basis="6-31(d,p)", dispersion=1, solvent="Methanol", charge=0, multiplicity=1, nproc=28,mem=100):
    ...
    with open("gs_sp/gs_sp.gjf","w") as f:
        for line in header:
            f.write(line+"\n")
        if dispersion == 1:
            f.write("#n %s/%s empiricaldispersion=gd3 Geom=Check Guess=Read SCRF=(Solvent=%s,NonEquilibrium=Read) NoSymm\n" %(functional, basis, solvent))
        else:
            f.write("#n %s/%s Geom=Check Guess=Read SCRF=(Solvent=%s,NonEquilibrium=Read) NoSymm\n"% (functional, basis, solvent))
        f.write("\n")
...
if __name__ == "__main__":
    ...
    gen_gs_geom_opt(coords, functional=args.functional, basis=args.basis, dispersion=args.dispersion, solvent=args.solvent,
        charge=args.charge, multiplicity=args.multiplicity, nproc=args.nproc, mem=args.mem) # step 1
    gen_vert_exc(nstates=6, functional=args.functional, basis=args.basis, dispersion=args.dispersion, solvent=args.solvent,
        charge=args.charge, multiplicity=args.multiplicity, nproc=args.nproc, mem=args.mem) # step 3
    gen_exc_geom(nstates=6, functional=args.functional, basis=args.basis, dispersion=args.dispersion, solvent=args.solvent,
        charge=args.charge, multiplicity=args.multiplicity, nproc=args.nproc, mem=args.mem) # step 4
    gen_exc_solv(nstates=6, functional=args.functional, basis=args.basis, dispersion=args.dispersion, solvent=args.solvent,
        charge=args.charge, multiplicity=args.multiplicity, nproc=args.nproc, mem=args.mem) # step 5
    gen_gs_sp(functional=args.functional, basis=args.basis, dispersion=args.dispersion, solvent=args.solvent,
